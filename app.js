...
app.view('fido_issue_modal', async ({ ack, body, view, client }) => {
  ...
  try {
    ...
    // Create ClickUp task
    const modalData = {
      ticketId, property, clientName, market: market.toLowerCase(), issueType, priority: priority.toLowerCase(),
      description, source, sourceDetails, dateStr
    };
...
app.view('fido_inquiry_modal', async ({ ack, body, view, client }) => {
  ...
  try {
    ...
    // Create ClickUp task
    const modalData = {
      ticketId, property, clientName, market: market.toLowerCase(), inquiryType, priority: priority.toLowerCase(),
      details, source: method, sourceDetails: contactRef, dateStr
    };
...
app.view('fido_unit_change_modal', async ({ ack, body, view, client }) => {
  ...
  try {
    ...
    // Create ClickUp task
    const modalData = {
      ticketId, property, clientName, market: market.toLowerCase(), changeType: changeVal,
      trashDay: trashVal, recyclingDay: v.recycling_day_block?.recycling_day_select?.selected_option?.value,
